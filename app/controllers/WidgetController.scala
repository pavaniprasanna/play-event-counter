 ...
 * for details.
 */
class WidgetController @Inject()(cc: MessagesControllerComponents, counter: EventCounter) extends MessagesAbstractController(cc) {
  import WidgetForm._
  ...
  def count = Action { Ok(counter.addGetCount()) }
  ...
  def createWidget = Action { implicit request: MessagesRequest[AnyContent] =>
    ...
    val successFunction = { data: Data =>
      ...
      val widget = Widget(time = data.time, eventcount = counter.getCount(data.time))
